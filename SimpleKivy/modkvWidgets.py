...
class TextInput(FocusBehavior, Widget):
    ...
    def __init__(self, **kwargs):
        ...
        # self.register_event_type('on_text_escape')
    ...
    def _unbind_keyboard(self):
        ...
        utils.Window.unbind(on_textedit=self.window_on_textedit)
...
def __getattr__(name):
    if name=='CodeInput':
        class _CodeInput(CodeNavigationBehavior, utils.TextInput):
            ...
            def __init__(self, **kwargs):
                ...
                super(_CodeInput, self).__init__(**kwargs)
            ...
            def _get_line_options(self):
                kw = super(_CodeInput, self)._get_line_options()
                kw['markup'] = True
                kw['valign'] = 'top'
        ...
        class ICell(utils.TextInput):
            ccell=ObjectProperty()
            ccolors=utils.CaseInsensitiveDict()
            # check=Clock.schedule_once(self.check_next)
            def on_text_validate(self,*args):
                # print(self,args)
                # self.parent.hide_widget(self)
                self.ccell._text=self.text
            ...
            def on_focus(self,instance,value):
                ...
                if not value:
                    ...
                    # Clock.schedule_once(self.check_next)
                    self.decolor_text()
                else:
                    self.color_text(self,self.text)

                    # self.height, self.size_hint_y, self.opacity, self.disabled = 0, None, 0, True
            def check_next(self,dt):
                # print(self.disabled)
                self.height, self.size_hint_y, self.opacity, self.disabled = 0, None, 0, True
                # self.on_text_validate()
            def color_text(self,instance,text):
                text=text.strip()
                if not '=' in text[:1]:
                    return
                self.ccolors.clear()

                mcells= utils.find_letter_number_pairs(text)
                ...
                ls=re.findall(r"[a-z][0-9]+:[a-z][0-9]+",text,flags=re.IGNORECASE)
                for li in ls:
                    l,r=li.split(':')
                    tl=re.sub(r"\d", "", l)
                    il=re.sub(r"\D", "", l)
                    ...
                    abc0=utils.abc.index(tl.upper())
                    abc1=utils.abc.index(tr.upper())
                    il=int(il)
                    ir=int(ir)
